...
CREATE TABLE
    sellsbilling(
        ...
        totalcharges DOUBLE AS(freightcharges + labourcharges),
...
CREATE PROCEDURE APPLYLABOURCHARGES(IN BILLID INT) 
BEGIN 
	DECLARE labourRate DOUBLE DEFAULT 0;
	DECLARE Quantity INT DEFAULT 0;
	DECLARE collection_Id INT ;
	SELECT
	    collectionid INTO collection_Id
	FROM billing
	WHERE id = billId;
	SELECT rate INTO labourRate
	FROM labourrates, collections
	WHERE
	    collections.id = collection_Id
	    AND labourrates.containertype = collections.containertype;
	SELECT
	    collections.quantity INTO Quantity
	FROM collections
	WHERE
	    collections.id = collection_Id;
	UPDATE billing
	SET
	    labourcharges = labourRate * Quantity
	WHERE id = billId;
END; 
...
CREATE PROCEDURE DEDUCTLABOURCHARGESFROMREVENUE(BILLID 
INT) BEGIN 
	...
	WHERE id = billId;
	SELECT
	    labourcharges INTO labour_Charges
	FROM billing
	WHERE id = billId;
	SELECT
	    collections.netweight * collections.rateperkg INTO revenue
	FROM collections
	WHERE id = collection_Id;
	UPDATE billing
	SET
	    totalamount = revenue - labour_Charges
	WHERE id = billId;
END; 
...
BEGIN 
	...
	SELECT
	    freightrates.kilometers * freightrates.rateperkm INTO totalFreightCharges
	FROM freightrates
	WHERE
	    freightrates.billid = billId;
	UPDATE sellsbilling
	SET
	    freightcharges = totalFreightCharges
	WHERE sellsbilling.id = billId;
END; 

-- DROP PROCEDURE calculate_labour_charges_of_sells;

CREATE PROCEDURE APPLYLABOURCHARGESTOBILLING(IN BILLID 
INT) BEGIN 
	UPDATE sellsbilling
...
CREATE PROCEDURE APPLYTOTALAMOUNT(BILLID INT) BEGIN 
	...
	SELECT
	    totalcharges INTO total_charges
	FROM sellsbilling
	WHERE id = billId;
	SELECT
	    sells.netweight * sells.rateperkg INTO totalAmount
	FROM sells
	WHERE id = sell_id;
	UPDATE sellsbilling
	SET
	    totalamount = totalAmount - total_charges
	WHERE id = billId;
END; 

--   DROP PROCEDURE makepayment;

CREATE PROCEDURE MAKEPAYMENT(IN BILLID INT, OUT OWNER_ID 
INT, OUT FARMER_ID INT, OUT VENDOR_ID INT, OUT OWNER_AMOUNT 
DOUBLE, OUT FARMER_AMOUNT DOUBLE, OUT VENDOR_AMOUNT 
DOUBLE) BEGIN 
	SET owner_id= (SELECT userid from userroles WHERE roleid=1);
	SET farmer_id = (
	        SELECT farmerid
	        from collections
	            INNER JOIN sells on collections.id = sells.collectionid
	...
	SET owner_amount = (
	        ...
	        SELECT labourcharges
	        FROM sellsbilling
	        WHERE
	            id = billid
	    ) - farmer_amount;
	SET vendor_amount = (
	        SELECT freightcharges
	        FROM sellsbilling
	        WHERE id = billid
	    );
END; 
...
INSERT INTO
    labourrates(containertype, imageurl, rate)
VALUES
(
        'crates',
...
(
        ...
        'Sahil',
        'Mankar',
        '/assets/images/sahilmankar.jpeg',
        '565645455676'
    );
...
VALUES (
        "OM Transports",
        '/assets/images/goodstransport.jpeg',
        20
    );
...
VALUES (
        "Navale Transport",
        '/assets/images/goodstransport1.jpeg',
        21
    );
...
VALUES (
        "Karale Transport",
        '/assets/images/goodstransport2.jpeg',
        22
    );
...
VALUES (
        "Sakore Transport",
        '/assets/images/sakoretransport.jpeg',
        23
    );
...
VALUES (
        ...
        63,
        '2023-06-27'
    ), (23, 24, 5, 9, 45, 52, '2023-01-06'), (24, 25, 6, 6, 30, 43, '2023-06-23'), (
        25,
        26,
        1,
        16,
        ...
        63,
        '2023-02-20'
    ), (23, 24, 1, 9, 45, 52, '2023-06-14'), (24, 25, 1, 6, 30, 43, '2023-06-14'), (
        25,
        26,
        1,
        16,
...
VALUES ('Mumbai', 'Pune', 150, 25, 1), ('Mumbai', 'Nagpur', 800, 20, 2), ('Mumbai', 'Nashik', 170, 15, 3), ('Mumbai', 'Kolhapur', 380, 28, 4), (
        'Mumbai',
        'Aurangabad',
        350,
        25,
        5
    ), ('Mumbai', 'Solapur', 440, 24, 6), ('Mumbai', 'Amravati', 790, 28, 7), ('Mumbai', 'Jalgaon', 420, 29, 8), ('Mumbai', 'Akola', 660, 30, 9), (
        'Mumbai',
        'Ratnagiri',
        340,
        32,
        ...
        36,
        12
    ), ('Mumbai', 'Sangli', 410, 40, 13), ('Mumbai', 'Mumbai', 49, 47, 14), ('Mumbai', 'Thane', 30, 59, 15), ('Mumbai', 'Nagpur', 800, 69, 16), ('Mumbai', 'Pune', 150, 80, 17), ('Mumbai', 'Nashik', 170, 68, 18), (
        'Mumbai',
        'Kolhapur',
        380,
        15.8,
        ...
        16.0,
        28
    ), ('Mumbai', 'Mumbai', 0, 0.0, 29), ('Mumbai', 'Thane', 30, 5.0, 30), (
        'Mumbai',
        'Nagpur',
        800,
        20.2,
...
CREATE PROCEDURE CALL_PROCEDURES(IN RECORDS INT) BEGIN 
	DECLARE i INT DEFAULT 1;
	WHILE i <= records DO CALL ApplyLabourCharges(i);
...
CREATE PROCEDURE CALL_PROCEDURESOFSELLS(IN RECORDS 
INT) BEGIN 
	DECLARE i INT DEFAULT 1;
	WHILE i <= records DO CALL ApplyFreightCharges(i);
...
SELECT
    sellsbilling.freightcharges,
    freightrates.fromdestination,
    freightrates.todestination,
    freightrates.kilometers,
    sells.quantity,
    freightrates.rateperkm
from vehicles
    INNER JOIN sells ON vehicles.id = sells.vehicleid
    INNER JOIN sellsbilling ON sells.id = sellsbilling.sellid
    INNER JOIN freightrates ON sellsbilling.id = freightrates.billid
WHERE vehicles.id = 1;
...
SELECT
    ...
    crops.title,
    collections.containertype,
    collections.grade,
    collections.quantity,
    collections.totalweight,
    collections.tareweight,
    collections.netweight,
    collections.rateperkg,
    collections.date
FROM users
    INNER JOIN collections ON users.id = collections.farmerid
    INNER JOIN crops ON crops.id = collections.cropid
WHERE
    collections.date >= CURRENT_DATE;
...
WHERE
    ...
    AND addresses.district = 'Sangli'
    AND addresses.tahsil = 'Jat'
    AND addresses.village = 'Bhavadi';
...
SELECT
    ...
    users.aadharid
FROM vendors
    INNER JOIN users on vendors.transportid = users.id
...
SELECT
    ...
    vendors.companyname
FROM users
    INNER JOIN vendors on vendors.transportid = users.id
WHERE users.id = 21;
...
SELECT
    ...
    `c`.`grade` AS `Grade`,
    `s`.`netweight` AS `NetWeight`,
    `s`.`rateperkg` AS `RatePerKg`,
...
FROM sells
    ...
    INNER JOIN users on sells.merchantid = users.id
WHERE vehicles.id = 3;
...
FROM sells
    ...
    INNER JOIN vehicles ON sells.vehicleid = vehicles.id
WHERE
    vehicles.vehiclenumber = 'MH14RE3456'
    and sells.`date` = '2023-06-14'
GROUP BY crops.title;

SELECT
    `c0`.`imageurl` AS `CropImage`,
    `l`.`imageUrl` AS `ContainerImage`,
    `s`.`quantity` AS `Quantity`,
    `c`.`grade` AS `Grade`,
    `s`.`netweight` AS `NetWeight`,
    `s`.`rateperkg` AS `RatePerKg`,
