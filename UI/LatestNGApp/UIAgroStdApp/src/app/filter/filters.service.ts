...
import { CollectionmanagerService } from '../Services/collectionmanager.service';
...
export class FiltersService {
  ...
  constructor(private http: HttpClient, private usersvc: UserService, private userrole: UserroleService,
    private merchantsvc: MerchantService, private corpsvc: CorporateService,
    private collectionCentersvc: CollectioncenterService, private transportersvc: TransporterService,private collectionmanager:CollectionmanagerService) { }
  ...
  getFarmers(): Observable<NameId[]> {
    return this.userrole.getusersId("farmer").pipe(
      switchMap((res) => this.usersvc.getUserNamesWithId(res))
    );
  }
  getInspectors(): Observable<NameId[]> {
    return this.userrole.getusersId("inspector").pipe(
      switchMap((res) => this.usersvc.getUserNamesWithId(res)
      )
    );
  }
  ...
  getCollectionCenters(): Observable<Corporate[]> {
    ...
    return this.collectionmanager.getCollectionCenterAndCorporateId().pipe(
      switchMap((res) => {
  ...
  //   return this.transportersvc.getCorporateIdOfTransporter().pipe(
  //     switchMap((res) => {
