...
export class TransporterbarchartComponent implements OnInit {
  ...
  fetchVehicleRevenues(year: number): void {
    this.svc
      .getVehicleRevenues(Number(this.transporterId), year)
      .subscribe((res) => {
        this.vehicleRevenues = res;
        this.barChartData.labels = this.vehicleRevenues.map(
          (revenues) => revenues.rtoNumber
        );
        this.barChartData.datasets[0].data = this.vehicleRevenues.map(
          (revenues) => revenues.amount
        );
        this.barChartData.datasets[0].backgroundColor = this.getBarColors(
          this.vehicleRevenues.length
        );
      });
  }
